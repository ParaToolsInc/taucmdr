...
layout = html.Div(children=[
    ...
    dcc.Dropdown(id='heatMap-dm-dropdown', clearable=False, options=config.option_metric_list,value='Exclusive'), 
    dbc.Row([dbc.Col(children=dcc.Graph(id='single-heat-map')),dbc.Col(children=dcc.Graph(id='path-heat-map'))])
])

@app.callback(
    [Output('single-heat-map', 'figure'),Output('path-heat-map','figure')],Input('heatMap-dm-dropdown', 'value'))
def make_heat_map(metricNum):
    """
    ...
    if len(callpaths)==0:
        ...
        callpaths = trim_data(callpaths, num_threads, TOP_N, metricNum)   #col_name == Exclusive
        data = create_func_data(data_flat)
        data = trim_data(data, num_threads, TOP_N, metricNum)
