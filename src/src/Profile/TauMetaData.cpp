...
static int writeMetaData(Tau_util_outputDevice *out, bool newline, int counter, int tid)
{
  ...
  if (tid == 0) {
    ...
    for (MetaDataRepo::iterator it = Tau_metadata_getMetaData(0).begin(); it != Tau_metadata_getMetaData(0).end();
        it++) {
    ...
    for (MetaDataRepo::iterator it = Tau_metadata_getMetaData(tid).begin(); it != Tau_metadata_getMetaData(tid).end();
        it++) {
      (*localRepo)[it->first] = it->second;
    }
  }
...
extern "C" void Tau_context_metadata(const char *name, const char *value)
{
  ...
  if (current) {
    ...
    string fname = fi->GetName() + " " + fi->GetType();
    key->timer_context = fname.c_str();
...
Tau_util_outputDevice *Tau_metadata_generateMergeBuffer()
{
  ...
  for (MetaDataRepo::iterator it = Tau_metadata_getMetaData(RtsLayer::myThread()).begin();
      it != Tau_metadata_getMetaData(RtsLayer::myThread()).end(); ++it) {
    const char *name = it->first.name;
