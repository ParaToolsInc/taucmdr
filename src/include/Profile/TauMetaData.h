...
struct Tau_metadata_key
{
  char * name;
  char const * timer_context;
  int call_number;
  x_uint64 timestamp;
  Tau_metadata_key() :
      name(NULL), timer_context(NULL), call_number(0), timestamp(0)
  {
  }
};
...
public:
  void emptyRepo(void)
  {
    ...
    while (it != this->end()) {
      ...
      if (eraseme->first.timer_context) free((void*)eraseme->first.timer_context);
