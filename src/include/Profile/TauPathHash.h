...
private:
  ...
  {
    struct HashElement *newEntry = (struct HashElement *)Tau_MemMgr_malloc(tid, sizeof(struct HashElement));
    if (newEntry == NULL) {
      /* Memory allocation failed! */
      fprintf(stderr, "ERROR tid %d: Failed to create new hash element. Tau_MemMgr_malloc failed.\n", tid);
      return NULL;
    }
    struct KeyValuePair *newPair = (struct KeyValuePair *)Tau_MemMgr_malloc(tid, sizeof(struct KeyValuePair));
    if (newPair == NULL) {
      /* Memory allocation failed! */
...
public:
  ...
  TauPathHashTable(int tid_, int size_ = TAU_PATHHASH_DEFAULT_SIZE) :
      tid(tid_), tableSize(size_), numElements(0), disableGrow(false), iterPtr(NULL), iterCount(0), iterTblIdx(0)
  {

    table = (struct HashElement **)Tau_MemMgr_malloc(tid, sizeof(struct HashElement *) * tableSize);
