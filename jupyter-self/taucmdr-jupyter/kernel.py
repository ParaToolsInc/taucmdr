...
def get_list(source, dashboard_columns):
    ...
    for record in source:
        ...
        rows.append(row)
    keys = rows[0]
    return [dict(zip(keys, values)) for values in rows[1:]]
...
def get_project(proj):
    ...
    project['target'] = get_list(target, target_columns)
    project['applications'] = get_list(application, application_columns)
    project['measurements'] = get_list(measurement, measurement_columns)
    project['experiments'] = get_list(experiment, experiment_columns)
    return project
