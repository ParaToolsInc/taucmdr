...
RUN DEBIAN_FRONTEND=noninteractive set -v && \
        apt-get update && \
        apt-get install -y --no-install-recommends ca-certificates curl default-jre wget libz-dev make locales git gcc g++ gfortran mpi-default-bin mpi-default-dev && \
